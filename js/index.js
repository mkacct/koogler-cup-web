...
function updateVisuals(obj) {
	...
	if (obj.showEls) {
		...
		if (obj.status == 'error') { // special case with error message
			changeNoContentDesc('An error occurred, select the <i class="fas fa-redo-alt"></i> button to retry' + (obj.clientError ? '<br>Make sure you are connected to the internet' : ''));
		} else {
